...
func (mp *messageProcessor) sendClientUpdate(
	...
	if err := dst.chainProvider.SendMessagesToMempool(broadcastCtx, msgs, mp.memo, 0, ctx, nil); err != nil {
...
func (mp *messageProcessor) sendBatchMessages(
	...
	if err := dst.chainProvider.SendMessagesToMempool(broadcastCtx, msgs, mp.memo, 0, ctx, callbacks); err != nil {
		for _, t := range batch {
			dst.finishedProcessing <- t
		}
...
func (mp *messageProcessor) sendSingleMessage(
	...
	err := dst.chainProvider.SendMessagesToMempool(broadcastCtx, msgs, mp.memo, 0, ctx, callbacks)
	if err != nil {
		dst.finishedProcessing <- tracker
		errFields := []zapcore.Field{
			zap.String("path_name", src.info.PathName),
